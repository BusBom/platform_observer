...
const int STABLE_THRESHOLD =
    20;  // 정차로 판단하기 위한 연속 감지 프레임 수 ( 30 = 2초 )
 ...
 * 영역 내 흰색 판단 : 70 (상위 30%)
 * 버스 또는 물체 유무 판단 : 0.35
 * notch tolernace : hue(25), sat(40), val(80)
 */
...
void receive_roi_config_thread() {
  ...
  while (running.load()) {
    ...
    if (FD_ISSET(server_fd, &read_fds)) {
      ...
      std::cout << "🤝 CGI client connected. Receiving ROI data..."
                << std::endl;
  ...
  while (running.load()) {
    ...
    cv::Mat debug_frame = local_frame.clone();
    {
      ...
      if (!platform_rois.empty()) {
        cv::polylines(debug_frame, platform_rois, true, cv::Scalar(0, 255, 0),
                      2);
      }
    }
...
void process_bus_status(unsigned int platform_count, const bool* raw_status) {
  for (unsigned int i = 0; i < platform_count; ++i) {
    if (raw_status[i]) {    // 버스가 감지되면
      ...
      miss_counter[i]++;
      if (miss_counter[i] >=
          3) {  // 3프레임 이상 연속 감지 실패 시 감지 카운터 초기화
        detect_counter[i] = 0;
        stable_status[i] = 0;
      }
    }
...
void mask_thread() {
  ...
  while (running.load() || !warped_queue.empty()) {
    ...
    if (warped_queue.try_pop(frame)) {
      std::vector<cv::Mat> notched(local_platform_size),
          masked_1(local_platform_size), masked_2(local_platform_size);
      batchApplyNotchFilterHSVtoAll((*frame), notched, notch_target);
      remove_achromatic_areas(notched, masked_1);
...
int main(int argc, char* argv[]) {
  ...
  while (running.load()) {
    ...
    if (is_config_ready.load()) {
      ...
      if (masked_queue.try_pop(masked)) {
        ...
        for (unsigned int i = 0; i < PLATFORM_SIZE; i++) {
          std::cout << "  Platform " << i << ": "
                    << (stable_status[i] ? "BUS DETECTED" : "Empty")
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(10));  // CPU 과점유 방지용
  }
